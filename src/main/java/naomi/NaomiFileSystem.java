...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = NaomiFileSystem.MyFileSystemFactory.class)
public class NaomiFileSystem implements GFileSystem {
	 ...
	 */
	public NaomiFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + NaomiFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryFull<NaomiFileSystem>, GFileSystemProbeFull {

		@Override
		public NaomiFileSystem create(FSRL containerFSRL, FSRLRoot targetFSRL,
				ByteProvider byteProvider, File containerFile, FileSystemService fsService,
				TaskMonitor monitor) throws IOException, CancelledException {

			NaomiFileSystem fs = new NaomiFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
